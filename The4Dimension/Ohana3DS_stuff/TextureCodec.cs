...
namespace The4Dimension.Ohana
{
    class TextureCodec
    {
        private  int[] tileOrder = { 0, 1, 8, 9, 2, 3, 10, 11, 16, 17, 24, 25, 18, 19, 26, 27, 4, 5, 12, 13, 6, 7, 14, 15, 20, 21, 28, 29, 22, 23, 30, 31, 32, 33, 40, 41, 34, 35, 42, 43, 48, 49, 56, 57, 50, 51, 58, 59, 36, 37, 44, 45, 38, 39, 46, 47, 52, 53, 60, 61, 54, 55, 62, 63 };
        private  int[,] etc1LUT = { { 2, 8, -2, -8 }, { 5, 17, -5, -17 }, { 9, 29, -9, -29 }, { 13, 42, -13, -42 }, { 18, 60, -18, -60 }, { 24, 80, -24, -80 }, { 33, 106, -33, -106 }, { 47, 183, -47, -183 } };
        ...
        /// <returns></returns>
        public  Bitmap decode(byte[] data, int width, int height, RenderBase.OTextureFormat format)
        {
            ...
            return new TextureUtils().getBitmap(output.ToArray(), width, height);
        }
        ...
        #region "ETC1"
        private  byte[] etc1Decode(byte[] input, int width, int height, bool alpha)
        {
            byte[] output = new byte[(width * height * 4)];
            long offset = 0;
        ...
        private  byte[] etc1DecodeBlock(byte[] data)
        {
        ...
        private  Color etc1Pixel(uint r, uint g, uint b, int x, int y, uint block, uint table)
        {
            int index = x * 4 + y;
            uint MSB = block << 1;
        ...
        private  int[] etc1Scramble(int width, int height)
        {
